...
def torch_allclose(x, y, rtol=1.0e-5, atol=1.0e-8):
    """
    ...
    return np.allclose(
        x.detach().cpu().numpy(), y.detach().cpu().numpy(), rtol=rtol, atol=atol
    )
...
def tanh_rescale(x, x_min=-1.0, x_max=1.0):
...
class CarliniWagnerLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        wrong_logit = torch.max((1.0 - label_mask) * input, dim=1)[0]
        loss = -F.relu(correct_logit - wrong_logit + 50.0).sum()
        return loss
...
def batch_per_image_standardization(imgs):
    ...
    mean = imgs.view(imgs.shape[0], -1).mean(dim=1).view(imgs.shape[0], 1, 1, 1)
...
def batch_adjusted_stddev(imgs):
    ...
    std_min = 1.0 / imgs.new_tensor(imgs.shape[1:]).prod().float().sqrt()
