...
from adv_test.algo_adv.algo_adv.advertorch_algo.utils import calc_l2distsq
from adv_test.algo_adv.algo_adv.advertorch_algo.utils import calc_l1dist
...
class ElasticNetL1Attack(Attack, LabelMixin):
    ...
    def _loss_fn(self, output, y_onehot, l1dist, l2distsq, const, opt=False):
        ...
        if self.targeted:
            loss_logits = clamp(other - real + self.confidence, min=0.0)
        else:
            loss_logits = clamp(real - other + self.confidence, min=0.0)
